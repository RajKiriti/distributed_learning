...
for epoch in range(obj['global_epochs']):
	...
	if obj['is_attack'] == 1:
		...
		print('number of byzantine users are',len(local_byz_updates))
	...
	############ Aggregating in small groups using SMP #############
	numb_groups=len(local_updates)/obj['users_per_group']
	m_user=obj['users_per_group']
	...
	for i in array_range:
		...
		if j%m_user==0:
			...
			for key in w_copy.keys():
				local_updates_group[key]+=local_updates[i][key]
				local_updates_group[key]=local_updates_group[key]/m_user
	...
	local_updates=local_updates_final
	local_sizes=local_sizes_final
	if obj['is_defense'] == 1:
